...
import { Camera, ChevronDown, Clock3, X } from "lucide-react";
  ...
  return (
    <div className="fixed inset-0 z-[999] overflow-y-auto bg-black/20 backdrop-blur-[10px]">
      <div className="flex min-h-full items-center justify-center p-4 md:p-8">
        <div className="relative flex max-h-[calc(100vh-32px)] w-full overflow-hidden rounded-[28px] bg-white shadow-[0_30px_80px_rgba(39,19,16,0.18)] md:max-h-[calc(100vh-64px)] md:max-w-[760px] md:rounded-[38px]">
          <div className="relative hidden w-[40%] bg-[#301713] md:block">
            <img
          ...
          <div className="flex min-h-0 flex-1 flex-col">
            <div className="flex items-start justify-between px-5 pt-6 md:px-10 md:pt-10">
              ...
              <button
                onClick={onClose}
                className="flex h-9 w-9 items-center justify-center rounded-full bg-[#F4F4F3] text-[#271310] md:bg-transparent"
              >
                <X size={22} />
              </button>
            </div>
            ...
            >
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-5">
                ...
                <Field label="Preparation Time" error={errors.estimated_time}>
                  <div className="relative">
                    <input
                      type="number"
                      min={1}
                      ...
                      onChange={(e) =>
                        setData("estimated_time", e.target.value)
                      }
                      placeholder="15"
                      className="h-12 w-full rounded-[14px] border border-[#ECE8E6] bg-[#FAFAF9] px-4 pr-14 text-[14px] outline-none focus:ring-2 focus:ring-[#301713]/10 md:h-14 md:text-[15px]"
                      required
                    />
                    <span className="absolute right-4 top-1/2 -translate-y-1/2 text-[13px] font-bold text-[#5A4A47]">
                      min
                    </span>
                  </div>
                </Field>

                <Field label="Availability" error={errors.is_available}>
            ...
            <div className="grid grid-cols-2 gap-3 border-t border-[#F0ECEA] px-5 py-4 md:flex md:items-center md:justify-end md:gap-5 md:px-10 md:py-6">
              <button
                type="button"
                onClick={onClose}
                className="h-12 rounded-[14px] border border-[#ECE8E6] text-[13px] font-extrabold text-[#271310] md:border-0 md:px-2 md:text-[16px]"
              >
                Cancel
              </button>
              <button
                ...
                form="menuForm"
                disabled={processing}
                className="h-12 rounded-[14px] bg-[#301713] px-5 text-[13px] font-extrabold text-white shadow-[0_14px_28px_rgba(39,19,16,0.18)] disabled:opacity-50 md:px-10 md:text-[15px]"
              >
                {processing
                  ? "Saving..."
                  : editData
    ...
    >
      ...
      {imagePreview ? (
        ...
        <div className="flex h-[100px] flex-col items-center justify-center gap-2 p-4 text-center md:h-[160px]">
          <Camera
            size={26}
            className={dark ? "text-white/50" : "text-[#C8A96E]"}
          />
          <span
